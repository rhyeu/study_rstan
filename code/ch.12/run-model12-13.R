...
T <- 96
d <- as.matrix(read.csv('.../ch.12/input/data-2Dmesh.txt', header=FALSE))
I <- nrow(d)
J <- ncol(d)
rownames(d) <- 1:I
colnames(d) <- 1:J
...
TID <- read.csv('.../ch.12/input/data-2Dmesh-design.txt', header=FALSE)
data <- list(I=I, J=J, Y=d, T=T, TID=TID)

stanmodel <- stan_model(file='.../ch.12/model/model12-13.stan')
fit <- sampling(
...
save.image('.../ch.12/output/result-model12-13.RData')

stanmodel_b <- stan_model(file='.../ch.12/model/model12-13b.stan')
fit_b <- sampling(
