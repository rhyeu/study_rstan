...
d <- read.csv(file='…/ch.8/input/data-attendance-4-2.txt')
...
ggsave(file='…/ch.8/e/fig-ex4-person.png', plot=p, dpi=300, w=4, h=3)
...
p <- p + geom_histogram(color='grey20', binwidth=bw)
p <- p + geom_line(eval(bquote(aes(y=..count..*.(bw)))), stat='density')
ggsave(file='…/ch.8/e/fig-ex4-course.png', plot=p, dpi=300, w=4, h=3)
