...
fit <- stan(file='ch.4/model/model4-4.stan', data=data, seed=1234)
...
ggsave(file='ch.4/output/fig4-8-left-2.png', plot=p, dpi=300, w=4, h=3)

d_est <- data.frame.quantile.mcmc(x=X_new, y_mcmc=ms$y_new)
p <- ggplot.5quantile(data=d_est)
p <- p + geom_point(data=d, aes(x=X, y=Y), shape=1, size=3)
p <- customize.ggplot.axis(p)
ggsave(file='ch.4/output/fig4-8-right-2.png', plot=p, dpi=300, w=4, h=3)
